...
const groupedResults = events.reduce((acc, row) => {
  ...
  if (row.event_date) {
    if (!event.times[row.event_date]) {
      event.times[row.event_date] = {
        startTime: row.heure_debut || '',
        endTime: row.heure_fin || '',
      };
    }
    // Si jamais la date existe déjà, on pourrait vérifier si on doit update
    // (exemple: prendre les horaires les plus précises ou réécrire).
  }
...
const [eventMofifDays, setEventModifDays] = useState([]);
...
const handleDateModifChange = (e) => {
  const { name, value } = e.target;
  let newFormData;

  if (name === 'startDate') {
    // Si startDate change, on réinitialise endDate systématiquement
    newFormData = {
      ...newDates,
      startDate: value,
      endDate: '',
  ...
  const toDate = (dateStr) => new Date(dateStr.split('T')[0]);

  // Si les deux dates sont valides, on génère les jours
  if (
    newFormData.startDate &&
    newFormData.endDate &&
    toDate(newFormData.startDate) <= toDate(newFormData.endDate)
  ) {
    const start = new Date(newFormData.startDate);
    const end = new Date(newFormData.endDate);
    const days = [];
  ...
  setNewDates(newFormData);

  // (debug) affichage dans la console
  console.log('newDates', newFormData);
  console.log('oldDates', oldDates);

  console.log('OLD', oldDates)
  console.log('New', newDates)
};



const handleTimeModifChange = (e, day) => {
  const { name, value } = e.target;
  const dayTimes = newDates.times[day] || {};
...
setOldDates({
startDate: event.startDate.split('T')[0], // Convertit la date en format 'YYYY-MM-DD'
endDate: event.endDate.split('T')[0],     // Convertit la date en format 'YYYY-MM-DD'
times: Object.fromEntries(
  Object.entries(event.times).map(([day, times]) => [
    day.split('T')[0],
    {
      startTime: times.startTime.slice(0, 5), // Extrait 'HH:MM' de 'HH:mm:ss'
      endTime: times.endTime.slice(0, 5),     // Extrait 'HH:MM' de 'HH:mm:ss'
    },
  ])
),
});

// Mise à jour des nouvelles dates (avec formatage)
setNewDates({
startDate: event.startDate.split('T')[0], // Format 'YYYY-MM-DD'
endDate: event.endDate.split('T')[0],     // Format 'YYYY-MM-DD'
times: Object.fromEntries(
  Object.entries(event.times).map(([day, times]) => [
    day.split('T')[0],
    {
      startTime: times.startTime.slice(0, 5), // 'HH:MM'
      endTime: times.endTime.slice(0, 5),     // 'HH:MM'
    },
  ])
),
});
...
const toDate = (dateStr) => new Date(dateStr.split('T')[0]);
  ...
  if (
    event.startDate &&
    event.endDate &&
    toDate(event.startDate) <= toDate(event.endDate)
  ) {
          ...
          <form
            onSubmit={async (e) => {
              ...
              try {      
                ...
                const times1 = oldDates.times;
                const times2 = newDates.times;
                
                const dates1 = Object.keys(times1);
                const dates2 = Object.keys(times2);
                
                // ✅ Dates communes avec vérification des heures modifiées
                const commonDates = dates1
                  .filter(date => dates2.includes(date))
                  .map(date => {
                    const t1 = times1[date];
                    const t2 = times2[date];
                
                    const modified = (t1.startTime !== t2.startTime) || (t1.endTime !== t2.endTime);
                ...
                // 🔴 Dates supprimées (présentes uniquement dans event1)
                const removedDates = dates1
                  .filter(date => !dates2.includes(date))
                  .map(date => ({
                    date,
                    ...times1[date]
                ...
                // 🟢 Nouvelles dates (présentes uniquement dans event2)
                const newDatesInsertion = dates2
                  .filter(date => !dates1.includes(date))
                  .map(date => ({
                    date,
                    ...times2[date]
                  }));
                
                // 🔁 Extra : Les dates communes avec heures modifiées uniquement
                const modifiedTimes = commonDates.filter(item => item.modified);
                
                console.log("✅ Dates communes :", commonDates);
                console.log("🔁 Heures modifiées :", modifiedTimes);
                console.log("🔴 Dates supprimées :", removedDates);
                console.log("🟢 Nouvelles dates :", newDatesInsertion);
                console.log("🟢 Description :", newDates);


                
                const response = await axios.post(`${apiUrl}/events/updateEvent/`, { selectedEvent, modifiedTimes, removedDates, newDatesInsertion, newDates, all_images });
...
<div className="row">
        ...
        <div className="col-6">
          ...
          <input
            type="date"
            name="startDate"
            value={newDates.startDate ? newDates.startDate.split('T')[0] : ''}
            onChange={handleDateModifChange}
            className="form-control"
          />
        </div>
        <div className="col-6">
          ...
          <input
            type="date"
            name="endDate"
            value={newDates.endDate ? newDates.endDate.split('T')[0] : ''}
            onChange={handleDateModifChange}
            className="form-control"
            min={newDates.startDate ? newDates.startDate.split('T')[0] : ''}
          />
        </div>
      </div>
      ...
      {/* <pre>{JSON.stringify(selectedEvent.times, null, 2)}</pre> */}
      {/* <pre>{JSON.stringify(oldDates, null, 2)}</pre>



      <pre>{JSON.stringify(newDates, null, 2)}</pre>



      <pre>{eventDays.length}</pre> */}
            ...
            <div key={index} className="row mb-2">
              ...
              <div className="col-6">
                ...
                <input
                  type="time"
                  name="startTime"
                  value={newDates.times[day]?.startTime || ''}
                  onChange={(e) => handleTimeModifChange(e, day)}
                  className="form-control"
                />
              </div>
              <div className="col-6">
                ...
                <input
                  type="time"
                  name="endTime"
                  value={newDates.times[day]?.endTime || ''}
                  onChange={(e) => handleTimeModifChange(e, day)}
                  className="form-control"
                />
              </div>
