          ...
          const fetchData = async () => {
            try {
              const [rep1, rep2, rep3 , rep4] = await Promise.all([
                axios.get(`${apiUrl}/fashion_model/all`),
...
// return false;
  ...
  const [localisation, setLocalisation] = useState({ latitude: '', longitude: '' });
        ...
        (error) => {
          switch(error.code) {
            case error.PERMISSION_DENIED:
              setStatus('Permission refusée. Veuillez autoriser l\'accès à la géolocalisation.');
              break;
            case error.POSITION_UNAVAILABLE:
              setStatus('Position non disponible. Veuillez vérifier votre connexion ou vos paramètres.');
              break;
            case error.TIMEOUT:
              setStatus('La demande de géolocalisation a expiré. Veuillez réessayer.');
              break;
            default:
  ...
  const handleDelete = (modelId) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce modèle ?")) {
      ...
      axios.post(`${apiUrl}/fashion_model/deleteFashionModel`
        , {modelId} )
        .then((response) => {
  ...
  if (!navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        selectedModel.latitude = position.coords.latitude;
        selectedModel.longitude = position.coords.longitude;
...
// const handleClickM = (model) => {
//   if (model) {
//     navigate(`/admin/detail_mannequin/1/${model}`);
//   } else {
//     console.error("ID du mannequin invalide :", model);
//   }
// };
        ...
        <tbody>
          {paginatedModels.map((model) => (
            <tr key={model.id_mannequin}>
              ...
              <td>
                <a href={`/admin/detail_mannequin/1/${model?.id_mannequin}`}>
        ...
        <div style={styles.modalOverlay}>
          ...
          (
            ...
            <hr className="mt-4"/>
                        ...
                        <h6 className=" text-center">Position GPS</h6>


                          <div style={{display:'flex', flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                              <a onClick={getLocalisation} style={styles.submitButton} className="btn btn-outline-success">Obtenir</a>
                              {(localisation.latitude && localisation.longitude) ? (
                                 <a onClick={resetLocalisation} style={styles.submitButton} className="btn btn-outline-success">Effacer</a>
                            ):(
                              <a style={{ cursor:'not-allowed',color:'gray'}} className="btn btn-outline" >Effacer</a>
                            )}
                          </div>
                          ...
                          <p>{status}</p>
                            {localisation.latitude && localisation.longitude && (
                              <>
                                <p>
                                 <strong>Latitude</strong>  : {localisation.latitude}
                                </p>
                                <p>
                                  <strong> Longitude </strong>  : {localisation.longitude}
                                </p>
                              </>
            ...
            <div className="rx mt-4">
              <h6 className=" text-center">Réseaux Sociaux</h6>

                <div style={styles.formGroup}>
                        <label>Instagram :</label>
                        <input type="text"
                          onChange={(e) => handleInputChange("model", "instagram", e.target.value)}
                        placeholder="Lien du compte Instagram" style={styles.input} />
                  </div> 
                  <div style={styles.formGroup}>
                        <label>Youtube :</label>
                        <input type="text"
                          onChange={(e) => handleInputChange("model", "youtube", e.target.value)}
                        placeholder="Lien du compte Youtube" style={styles.input} />
                  </div> 
                  <div style={styles.formGroup}>
                        <label>TikTok :</label>
                        <input type="text"
                          onChange={(e) => handleInputChange("model", "tiktok", e.target.value)}
                        placeholder="Lien du compte TikTok" style={styles.input} />
                  </div> 
                  <div style={styles.formGroup}>
                        <label>LinkedIn :</label>
                        <input type="text"
                          onChange={(e) => handleInputChange("model", "linkedin", e.target.value)}
                        placeholder="Lien du compte LinkedIn" style={styles.input} />
                  </div> 
                  <div style={styles.formGroup}>
                        <label>Facebook :</label>
                        <input type="text"
                          onChange={(e) => handleInputChange("model", "facebook", e.target.value)}
                        placeholder="Lien du compte Facebook" style={styles.input} />
                  </div>             
 ...
 {isDetailModalOpen && (
      ...
      {isUpdateModalOpen && (
        ...
        <div style={styles.modalContent}>
         ...
         <h2 style={{fontSize : "18px" , textAlign : "center"}}>Modifier hotesse</h2>
          <form
            onSubmit={async (e) => {
              e.preventDefault();
                ...
                <div className="row">
                     ...
                     <hr className="mt-4"/>
                        ...
                        <h6 className=" text-center">Position GPS</h6>
                          ...
                          <div style={{display:'flex', flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                              <a onClick={updateLocalisation} style={styles.submitButton} className="btn btn-outline-success">Obtenir</a>
                              {(selectedModel.latitude && selectedModel.longitude) ? (
                                 <a onClick={(resetUpdateLocalisation)} style={styles.submitButton} className="btn btn-outline-success">Effacer</a>
                            ):(
                              <a style={{ cursor:'not-allowed',color:'gray'}} className="btn btn-outline" >Effacer</a>
                            )}
                          </div>
                          ...
                          <p>{status}</p>
                            {selectedModel.latitude && selectedModel.longitude && (
                              <>
                                <p>
                                 <strong>Latitude</strong>  : {selectedModel.latitude}
                                </p>
                                <p>
                                  <strong> Longitude </strong>  : {selectedModel.latitude}
                                </p>
                              </>
                     ...
                     <div className="rx mt-4">
                      <h6 className="text-center">Réseaux Sociaux</h6>
                        <div style={styles.formGroup}>
                          <label>Instagram :</label>
                          <input type="text"
                            value={selectedModel.instagram}
                            onChange={(e) => setSelectedModel({ ...selectedModel, instagram: e.target.value })} 
                          placeholder="Lien du compte Instagram" style={styles.input} />
                        </div>
                        <div style={styles.formGroup}>
                          <label>Youtube :</label>
                          <input type="text"
                            value={selectedModel.youtube}
                            onChange={(e) => setSelectedModel({ ...selectedModel, youtube: e.target.value })} 
                          placeholder="Lien du compte Youtube" style={styles.input} />
                        </div>
                        <div style={styles.formGroup}>
                          <label>TikTok :</label>
                          <input type="text"
                            value={selectedModel.tiktok}
                            onChange={(e) => setSelectedModel({ ...selectedModel, tiktok: e.target.value })} 
                          placeholder="Lien du compte TikTok" style={styles.input} />
                        </div>
                        <div style={styles.formGroup}>
                          <label>Linkedin :</label>
                          <input type="text"
                            value={selectedModel.linkedin}
                            onChange={(e) => setSelectedModel({ ...selectedModel, linkedin: e.target.value })} 
                          placeholder="Lien du compte Linkedin" style={styles.input} />
                        </div>
                        <div style={styles.formGroup}>
                          <label>Facebook :</label>
                          <input type="text"
                            value={selectedModel.facebook}
                            onChange={(e) => setSelectedModel({ ...selectedModel, facebook: e.target.value })} 
                          placeholder="Lien du compte Facebook" style={styles.input} />
                        </div>
                     </div>
