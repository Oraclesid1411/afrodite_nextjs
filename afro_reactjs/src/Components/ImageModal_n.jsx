...
const ImageModal = ({ image, data, onClose, onDataLoaded }) => {
  ...
  // useEffect(() => {
  //   // Set loading to true when the effect is triggered
  //   alert('test')
  //   setLoading(true);
  
  //   // Check if data exists and is an array with elements
  //   if (data && data.length > 0) {
  //     const fetchImagesWithContributors = async () => {
  //       // Use Promise.all to fetch data for all images in parallel
  //       const updatedImages = await Promise.all(
  //         data.map(async (imageData) => {
  //           // Fetch contributors
  //           const contributors = await fetchContributors(imageData.id_image);
  ...
  //           // Fetch image stats
  //           const stats = await fetchImageStats(imageData.id_image);
  //           console.log("stats");
  //           console.log(stats);
  
  //           // Fetch like state
  //           const like_state = await fetchhasLiked(imageData.id_image);
  //           console.log("like_state");
  //           console.log(like_state);
  
  //           // Return the updated image data with contributors, stats, and like state
  //           return {
  //             ...imageData,
  //             contributors,
  //             stats,
  ...
  //       // Set the images with contributors in the state
  //       setImagesWithContributors(updatedImages);
  
  //       // Set the current image index based on the initial `image` prop
  //       const initialIndex = updatedImages.findIndex(
  ...
  //     fetchImagesWithContributors();
  //   } else {
  //     // If no data is available, set loading to false
  //     setLoading(false);
  //   }
  // }, [data, image]); // Effect runs when `data` or `image` props change
  ...
  useEffect(() => {
    if (imagesWithContributors.length > 0) {
      ...
      console.log(currentImage)
      // alert('test')
      // Set contributors for the current image
      console.log('data list')
      console.log(currentImage.contributors)
      ...
      <div className="modal-content">
                    ...
                    <div className="btn_list">
                      <div className="like_btn">
                        <div className="post-actions">
                          ...
                          <button onClick={() => handleLike(currentImage.id_image)} disabled={currentImage?.like_state[0]?.has_liked === 1} className={currentImage?.like_state[0]?.has_liked === 1 ? "active_icon" : ""}>
        ...
        {imagesWithContributors.length > 0 && imagesWithContributors[currentImageIndex] && (
          <div className="details-section">
            ...
            <a href='/parteners' className='link_cst cstm_link mx-2 ' >
                                  collabs :
             </a>
              {photographe.length > 0 && (
                <label className="photographe">
                  ...
                  {photographe.map((item) => (
                    <a key={item?.collab_id} href={`/singleViewP/1/${item?.collab_id}`} className="value mx-1">{item?.collab_pseudo}</a>
                  ))}
              ...
              {styliste.length > 0 && (
                <label className="styliste">
                  ...
                  {styliste.map((item) => (
                    <a key={item?.collab_id} href={`/singleViewP/4/${item?.collab_id}`} className="value mx-1">{item?.collab_pseudo}</a>
                  ))}
              ...
              {maquilleur.length > 0 && (
                <label className="maquilleur">
                  ...
                  {maquilleur.map((item) => (
                    <a key={item?.collab_id} href={`/singleViewP/2/${item?.collab_id}`} className="value mx-1">{item?.collab_pseudo}</a>
                  ))}
              ...
              {coiffeur.length > 0 && (
                <label className="coiffeur">
                  ...
                  {coiffeur.map((item) => (
                    <a key={item?.collab_id} href={`/singleViewP/3/${item?.collab_id}`} className=" mx-1">{item?.collab_pseudo}</a>
                  ))}
