...
import ImageModal from './ImageModalO';
...
import {faShare,faCalendar, faImage,faFilm ,faList ,faThumbsUp,faHeart,faComment, faInfo} from '@fortawesome/free-solid-svg-icons';
                      ...
                      {/* <MannequinNavigation className="custom-MnavigationDesktop"/> */}
                        ...
                        <div className="photoRx">
                              ...
                              <div className="agent_rxsx">
                                ...
                                <button className='contactInfo contactInfo1 d-none btn btn-light me-2' onClick={openContactModal}>
                                  Contact Info
                                </button>
                        ...
                        <h6>{p?.nom} {p?.pseudo}</h6>
                          ...
                          <button className='btn btn-light btn-sm share me-5'>
                                <FontAwesomeIcon icon={faThumbsUp} size="sm" />
                          </button>

                        <button className='contactInfo contactInfo2 btn btn-light me-2' onClick={openContactModal}>
                          Contact Info
                        </button>
                            ...
                            {this_model.map((item) => (
                               ...
                               <div  key={item?.id_image} data-id={item.id_image} className="box col-lg-2 col-md-4 col-4 aos-init aos-animate" data-aos="fade-up" data-aos-duration="700">
                                <div className="product-card">
                                  ...
                                  <a className="hover-switch" data-fancybox="gallery"  href={`${apiUrl}/${item?.path_image}`} data-caption="Ceci est une description après le zoom"> {item.id_image}
                                  ...
                                  <div className="interaction_buttons" style={{display : "none"}}>
                                      ...
                                      >
                                         <button className='comment-button' onClick={async (event) => {
                                             
                                              console.log(item.id_image);

                                              const id = parseInt(item.id_image)

                                              
const fetchPhotographeData = async (value) => {

  console.log(value.type_contributeurs)


  try {
    const rep4 = await axios.post(`${apiUrl}/fashion_model/contribPhotographe`, {
      idPhotographe: value.type_contributeurs,
    });
    
    setPhotographe(rep4.data); 

    const pseudo = Photographe.pseudo;
...
try {
  ...
  const rep9 = await axios.post(`${apiUrl}/fashion_model/contrib`, { id_image: id });
  console.log(rep9);
  setContributeurs(rep9.data);

  if (constributeurs && constributeurs.length > 0) {
    for (const value of constributeurs) {
      if (value.type_contributeurs === 1) {
        await fetchPhotographeData(value);
      }
    }
  }
