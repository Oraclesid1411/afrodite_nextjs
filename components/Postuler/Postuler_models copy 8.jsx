...
<>
  ...
  {!extraOption &&
   ...
   <>
    ...
    <div className="custom-dropdown" ref={dropdownRef_more}>
      <button
        type="button"
      ...
      >
       {options_more.find(opt => opt.value === selectedOption_more)?.label || "Sélectionner"}
       </button>

      {isOpen_more && (
        <div className="custom-dropdown-menu">
          {options_more.map(option => (
            <div
              key={option.value}
              className={`custom-dropdown-item ${selectedOption_more === option.value ? "selected" : ""}`}
              onClick={() => handleSelect_more(option.value)}
            >
              {option.label}
            </div>
          ))}
